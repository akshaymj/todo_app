...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-green-200 p-4">
        <div class="lg:w-2/4 mx-auto py-8 px-6 bg-white rounded-xl" >
            <h1 class="font-bold text-5xl text-center mb-6">Todo App</h1>
            <div class="mb-6">
                <form class="flex flex-col space-y-4" method="POST" action="/">
                    @csrf
                    <input type="text" name="title" placeholder="Task Title" class="py-3 px-4 bg-gray-200 rounded-xl">
            ...
            <div class="mt-2">
                @foreach($todos as $todo)
                    <div 
                        ...
                        $todo->isDone ? 'bg-blue-300 rounded-xl' : ''
                        ])
                    >
                        <div class="flex-1 pr-8">
                            <h3 class="text-lg font-semibold">{{$todo->title}}</h3>   
                            <p>Task Created on {{date('Y-m-d',strtotime($todo->created_at))}}</p>
                        </div>
